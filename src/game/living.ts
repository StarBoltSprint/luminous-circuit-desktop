...
function pulseVeyraBreath(c, citizens) {
	...
	c.lastPulse = now;
	const pick = veyraBreathSite(c);
	setRoute(c, pick.x, pick.z);
	c.job = "watch";
	c.timer = 12;
	c.thought = "Veyra walks Hub breath — listening place, never a throne";
	c.intent = c.thought;
...
function pulseTalBridges(c, citizens) {
	...
	if (now - (c.lastPulse || c.lastHail || 0) < 40e3) return;
	c.lastPulse = now;
	const pick = talSpanSite(c);
	setRoute(c, pick.x, pick.z);
	c.job = "watch";
	c.timer = 14;
	c.thought = "Tal walks the span — both banks must believe, never a toll";
	c.intent = "Walking the span";
	noteLive(c, "watch", c.thought);
	let n = 0;
	for (const o of citizens) {
		if (o === c) continue;
...
function pulseAureParent(c, citizens) {
	...
	if (now - (c.lastPulse || c.lastHail || 0) < 46e3) return;
	c.lastPulse = now;
	const pick = aureOverlookSite(c);
	setRoute(c, pick.x, pick.z);
	c.job = "watch";
	c.timer = 12;
	c.thought = "Aure walks the overlook — parent still sits, never moved";
	c.intent = c.thought;
	noteLive(c, "watch", c.thought);
	let n = 0;
	for (const o of citizens) {
