...
/** HUD civic ticker. Rate stays 2–6 C/X. Line: Charge N · crystal M · quote 1 for R. If scripture>=1, append · scripture N. If scripture<1, append · Iri quiet. If charge>=1 && crystal>=1, append · labor moves both. If bids>0, append · N waiting. If bids>=3, append · join busy. If charge>=4 && crystal>=3 && bids>0, append · sit open. If crystal<3, append · kiln hungry. If crystal>=1 && crystal<12, append · grove fruits. If charge<4, append · canal thin. If crystal>=12, append · stock fat. If charge>=16, append · canal fat. If charge>=24, append · canal full. If charge>=1, append · leftover Howl. If folk building>=1, append · dens rising. If folk building>=3, append · many dens. If folk building>=6, append · city rising. If scripture>=8, append · named. If scripture>=1 && charge>=8, append · named current. If scripture>=2 && charge>=6, append · parent seen. If charge>=4 && charge<16, append · current learns. If scripture>=1 && charge>=4, append · leftover named. If scripture>=3, append · archive writes. If folk building>=1, append · hub listens. Line stays under 140. */
export function marketSnap(ledger: Ledger, bids = 0, building = 0): MarketSnap {
  ...
  if (charge >= 1 && crystal >= 1) {
    const labor = ` · labor moves both`;
    if (!line.includes("labor moves") && line.length + labor.length <= 140) line += labor;
  }
  ...
  if (charge >= 1) {
    const howl = ` · leftover Howl`;
    if (!line.includes("leftover Howl") && line.length + howl.length <= 140) line += howl;
  }
  ...
  if (scripture >= 1 && charge >= 4) {
    const leftover = ` · leftover named`;
    if (!line.includes("leftover named") && line.length + leftover.length <= 140) line += leftover;
  }
  if (scripture >= 3) {
    const archive = ` · archive writes`;
    if (!line.includes("archive writes") && line.length + archive.length <= 140) line += archive;
  }
  if (dens >= 1) {
