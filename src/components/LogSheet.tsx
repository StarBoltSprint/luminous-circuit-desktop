...
/** Aure walk-the-overlook labor. Parent stays. HUD job is the thought, not raw `watch`. */
function aimLaborPrefix(id: string, blob: string) {
  return id === "aure" && /parent|aim|horizon|overlook/i.test(blob) ? "Parent aim · " : "";
}

/** Iri walk-the-archive labor. Leftover light names. HUD job is the thought, not raw `watch`. */
function nameLaborPrefix(id: string, blob: string) {
...
/** Seln walk-the-canal labor. Tend leftover Howl, never bottled. HUD job is the thought, not raw `watch`. */
function canalLaborPrefix(id: string, blob: string) {
